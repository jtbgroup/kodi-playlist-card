...
public getThumbnailUrl(item: PlaylistItem): string | undefined {
    ...
    if (itemType === "song" || itemType === "music") {
      if (item.albumid) {
        return `/api/media_player_proxy/media_player.kodi/browse_media/album/${String(item.albumid)}`;  
      }
    }
    ...
    /**
     * Charge la miniature d'un élément (détermine l'URL + charge + convertit en base64)
     */
    public async loadThumbnailForItem(item: PlaylistItem, onUpdate: () => void): Promise<string | undefined> {
        const url = this.getThumbnailUrl(item);
        if (!url) return undefined;
        return await this.loadThumbnail(url, onUpdate);
    }
    ...
    public getThumbnailFromCache(url: string): string | undefined {
        return this._thumbnailCache.get(url);
    }
